...
from outhad_contextkit.configs.prompts import MEMORY_ANSWER_PROMPT
from outhad_contextkit.memory.telemetry import capture_client_event, capture_event

logger = logging.getLogger(__name__)


class Outhad_Contextkit:
    def __init__(
        self,
        ...
        if api_key:
            self.outhad_contextkit_client = MemoryClient(api_key, host)
        else:
            self.outhad_contextkit_client = Memory.from_config(config) if config else Memory()

        self.chat = Chat(self.outhad_contextkit_client)


class Chat:
    def __init__(self, outhad_contextkit_client):
        self.completions = Completions(outhad_contextkit_client)


class Completions:
    def __init__(self, outhad_contextkit_client):
        self.outhad_contextkit_client = outhad_contextkit_client

    def create(
        self,
        model: str,
        messages: List = [],
        # Outhad_Contextkit arguments
        ...
        if isinstance(self.outhad_contextkit_client, Memory):
            capture_event("outhad_contextkit.chat.create", self.outhad_contextkit_client)
        else:
            capture_client_event("outhad_contextkit.chat.create", self.outhad_contextkit_client)
        return response
    ...
    def _async_add_to_memory(self, messages, user_id, agent_id, run_id, metadata, filters):
        def add_task():
            ...
            self.outhad_contextkit_client.add(
                messages=messages,
                user_id=user_id,
                agent_id=agent_id,
                run_id=run_id,
    ...
    def _fetch_relevant_memories(self, messages, user_id, agent_id, run_id, filters, limit):
        ...
        return self.outhad_contextkit_client.search(
            query="\n".join(message_input),
            user_id=user_id,
            agent_id=agent_id,
            run_id=run_id,
    ...
    def _format_query_with_memories(self, messages, relevant_memories):
        # Check if self.outhad_contextkit_client is an instance of Memory or MemoryClient

        entities = []
        if isinstance(self.outhad_contextkit_client, outhad_contextkit.memory.main.Memory):
        ...
        elif isinstance(self.outhad_contextkit_client, outhad_contextkit.client.main.MemoryClient):
