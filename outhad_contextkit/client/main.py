...
from outhad_contextkit.client.project import AsyncProject, Project
from outhad_contextkit.client.utils import api_error_handler
from outhad_contextkit.memory.setup import get_user_id, setup_config
from outhad_contextkit.memory.telemetry import capture_client_event
...
class MemoryClient:
    """Client for interacting with the Outhad_Contextkit API.

    This class provides methods to create, retrieve, search, and delete
    memories using the Outhad_Contextkit API.

    Attributes:
        api_key (str): The API key for authenticating with the Outhad_Contextkit API.
        host (str): The base URL for the Outhad_Contextkit API.
    ...
    def __init__(
        ...
        Args:
            api_key: The API key for authenticating with the Outhad_Contextkit API. If not
                     provided, it will attempt to use the OUTHAD_CONTEXTKIT_API_KEY
                     environment variable.
            host: The base URL for the Outhad_Contextkit API. Defaults to
                  "https://api.outhad_contextkit.ai".
        ...
        """
        self.api_key = api_key or os.getenv("OUTHAD_CONTEXTKIT_API_KEY")
        self.host = host or "https://api.outhad_contextkit.ai"
        self.org_id = org_id
        self.project_id = project_id
        self.user_id = get_user_id()

        if not self.api_key:
            raise ValueError("Outhad_Contextkit API Key not provided. Please provide an API Key.")
            ...
            self.client.headers.update(
                {
                    ...
                    "Outhad_Contextkit-User-ID": self.user_id,
                }
            )
        else:
        ...
        if kwargs.get("output_format") != "v1.1":
            ...
            warnings.warn(
                (
                    ...
                    "https://docs.outhad_contextkit.ai/platform/quickstart#4-1-create-memories"
                ),
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def chat(self):
        """Start a chat with the Outhad_Contextkit AI. (Not implemented)
...
class AsyncMemoryClient:
    """Asynchronous client for interacting with the Outhad_Contextkit API.
