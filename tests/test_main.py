...
def memory_instance():
    with (
        patch("outhad_contextkit.utils.factory.EmbedderFactory") as mock_embedder,
        patch("outhad_contextkit.memory.main.VectorStoreFactory") as mock_vector_store,
        patch("outhad_contextkit.utils.factory.LlmFactory") as mock_llm,
        patch("outhad_contextkit.memory.telemetry.capture_event"),
        patch("outhad_contextkit.memory.graph_memory.MemoryGraph"),
    ):
...
def test_custom_prompts(memory_custom_instance):
    ...
    with patch("outhad_contextkit.memory.main.parse_messages", return_value="Test message") as mock_parse_messages:
        with patch(
            "outhad_contextkit.memory.main.get_update_memory_messages", return_value="custom update memory prompt"
